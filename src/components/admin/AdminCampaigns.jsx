...
const AdminCampaigns = () => {
  ...
  const toggleRowExpansion = async (campaignId) => {
    ...
    if (isExpanding && !campaignLinks[campaignId]) {
      ...
      try {
        // Fetch links for this campaign using admin API
        const response = await api.adminLinks.getAll({ campaign_id: campaignId });
  ...
  const filteredCampaigns = campaigns.filter(campaign => {
    ...
    expandableContent: (
      <div className="space-y-4">
        ...
        {/* Inline Links Table (Admin View) */}
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Campaign Links (Admin View)</CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  <tbody>
                    {campaignLinks[campaign.id].map(link => (
                      <tr key={link.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="px-4 py-2 text-muted-foreground">{link.owner || campaign.owner || 'Unknown'}</td>
