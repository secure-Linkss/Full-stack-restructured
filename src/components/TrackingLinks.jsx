...
const QRModal = ({ link, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-sm mx-4 p-6 relative shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-center bg-white rounded-xl p-4 min-h-[200px] mb-4">
          {loading ? (
            <div className="relative w-9 h-9"><div className="absolute inset-0 rounded-full border-2 border-blue-500/20" /><div className="absolute inset-0 rounded-full border-2 border-transparent border-t-blue-500 animate-spin" /></div>
          ) : qrDataUrl ? (
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            ...
            <select className="enterprise-input w-full" style={glassInput} value={format} onChange={e => setFormat(e.target.value)}>
              <option value="png">PNG</option>
              <option value="svg">SVG</option>
            </select>
          </div>
          <div>
            <label className="text-xs text-white/40 block mb-1">Size (px)</label>
            <select className="enterprise-input w-full" style={glassInput} value={size} onChange={e => setSize(Number(e.target.value))}>
...
const HealthModal = ({ link, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-md mx-4 p-6 relative shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold text-white flex items-center gap-2">
        ...
        <div className="p-4 mb-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40 mb-1">Current Status</p>
            ...
            <div className="text-right">
              <p className="text-xs text-white/40">Response Code</p>
              <p className="text-2xl font-heading font-bold text-white">{data?.health_response_code || '—'}</p>
            </div>
          </div>
          {data?.health_last_checked && (
            <p className="text-xs text-white/40 mt-2">Last checked: {new Date(data.health_last_checked).toLocaleString()}</p>
          )}
        </div>
        ...
        {data?.logs?.length > 0 && (
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40 mb-2">Recent Logs</p>
            <div className="space-y-2">
              {data.logs.map((log, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg px-3 py-2" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                  ...
                  <span className="text-xs text-white/40">{log.response_code || '—'}</span>
                  <span className="text-xs text-white/40">{log.response_time_ms ? `${log.response_time_ms}ms` : '—'}</span>
                  <span className="text-xs text-white/40">{log.checked_at ? new Date(log.checked_at).toLocaleTimeString() : ''}</span>
                </div>
              ))}
            </div>
          </div>
...
const PixelModal = ({ link, onClose }) => {
  ...
  const PixelRow = ({ label, idKey, enableKey, placeholder }) => (
    <div className="p-4 space-y-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-white">{label}</span>
        <label className="flex items-center gap-2 cursor-pointer">
          <span className="text-xs text-white/40">{pixels[enableKey] ? 'Enabled' : 'Disabled'}</span>
          <div
...
const OGModal = ({ link, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-md mx-4 p-6 relative shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold text-white flex items-center gap-2">
        ...
        <div className="rounded-xl overflow-hidden mb-5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          {og.og_image_url && (
          ...
          <div className="p-3">
            <p className="text-sm font-semibold text-white truncate">{og.og_title || 'Page Title'}</p>
            <p className="text-xs text-white/40 mt-0.5 line-clamp-2">{og.og_description || 'Page description will appear here...'}</p>
          </div>
        </div>

        <div className="space-y-3 mb-5">
          <div>
            <label className="text-xs text-white/40 block mb-1">og:title</label>
            <input className="enterprise-input w-full" style={glassInput} placeholder="Social share title" value={og.og_title || ''} onChange={e => setOg(p => ({ ...p, og_title: e.target.value }))} maxLength={255} />
          </div>
          <div>
            <label className="text-xs text-white/40 block mb-1">og:description</label>
            <textarea className="enterprise-input w-full resize-none" style={glassInput} rows={3} placeholder="Social share description" value={og.og_description || ''} onChange={e => setOg(p => ({ ...p, og_description: e.target.value }))} maxLength={500} />
          </div>
          <div>
            <label className="text-xs text-white/40 block mb-1">og:image URL</label>
            <input className="enterprise-input w-full" style={glassInput} placeholder="https://..." value={og.og_image_url || ''} onChange={e => setOg(p => ({ ...p, og_image_url: e.target.value }))} />
          </div>
        </div>
...
const RoutingModal = ({ link, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-lg mx-4 p-6 relative shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold text-white flex items-center gap-2">
        ...
        <p className="text-xs text-white/40 mb-4">Rules are evaluated top-to-bottom. First match wins. Non-matching traffic goes to the default target URL.</p>

        {rules.length === 0 && (
          <div className="text-center py-8 text-white/40 text-sm">No rules yet. Add one below.</div>
        )}

        <div className="space-y-3 mb-4">
          {rules.map((rule, i) => (
            <div key={i} className="p-3 space-y-2 rounded-xl" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-white/40">Rule #{i + 1}</span>
                <button onClick={() => removeRule(i)} className="p-1 text-white/40 hover:text-[#ef4444]"><X className="w-3.5 h-3.5" /></button>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs text-white/40">Trigger</label>
                  <select className="enterprise-input w-full mt-1" style={glassInput} value={rule.type} onChange={e => updateRule(i, 'type', e.target.value)}>
                    {RULE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-xs text-white/40">Condition / Value</label>
                  <input className="enterprise-input w-full mt-1" style={glassInput} placeholder={
              ...
              <div>
                <label className="text-xs text-white/40">Destination URL</label>
                <input className="enterprise-input w-full mt-1" style={glassInput} placeholder="https://destination.com" value={rule.destination_url || ''} onChange={e => updateRule(i, 'destination_url', e.target.value)} />
              </div>
            </div>
          ))}
        </div>
...
const BulkImportModal = ({ onClose, onImported }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-lg mx-4 p-6 relative shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-heading font-semibold text-white flex items-center gap-2">
        ...
        <div
          className="border-2 border-dashed rounded-xl p-8 text-center mb-4 cursor-pointer transition-colors hover:border-[#3b82f6]/50"
          style={{ borderColor: 'rgba(255,255,255,0.08)' }}
          onClick={() => fileInputRef.current?.click()}
        >
          <FileText className="w-8 h-8 text-white/20 mx-auto mb-2" />
          <p className="text-sm font-medium text-white">{csvFile ? csvFile.name : 'Click to upload CSV'}</p>
          <p className="text-xs text-white/40 mt-1">Required columns: <code className="bg-white/5 px-1 rounded">target_url</code> — Optional: <code className="bg-white/5 px-1 rounded">campaign_name</code></p>
          <input ref={fileInputRef} type="file" accept=".csv,text/csv" className="hidden" onChange={onFileChange} />
        </div>

        {preview.length > 0 && (
          <div className="mb-4">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40 mb-2">Preview (first {preview.length} rows)</p>
...
const TrackingLinks = () => {
  ...
  const metricCards = [
    { title: 'Total Tracking Links', value: links.length.toLocaleString(), icon: LinkIcon, accent: '#3b82f6' },
    { title: 'Total Clicks', value: metrics.totalClicks.toLocaleString(), icon: BarChart3, accent: '#10b981' },
    { title: 'Real Visitors', value: metrics.realVisitors.toLocaleString(), icon: Users, accent: '#10b981' },
    { title: 'Bots Blocked', value: metrics.botsBlocked.toLocaleString(), icon: Shield, accent: '#ef4444' },
  ];
  ...
  return (
    <div className="animate-fade-in w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-heading text-white">Tracking Links</h2>
          <p className="text-sm text-white/40 mt-1">Manage infrastructure routing and pixels</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
        {metricCards.map((card, i) => (
          <motion.div
            ...
            style={{ ...glassCard, overflow: 'hidden', position: 'relative' }}
            className="p-5 flex justify-between items-center"
          >
            {/* Accent strip */}
            <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: card.accent, borderRadius: '14px 0 0 14px' }} />
            <div className="pl-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-white/40">{card.title}</p>
              <h3 className="text-3xl font-heading text-white mt-1.5 tabular-nums-custom">{card.value}</h3>
            </div>
            <div className="p-3 rounded-xl" style={{ background: `${card.accent}18` }}>
      ...
      {/* Control Bar */}
      <div style={glassCard} className="p-4 flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
          <input
            type="text"
            placeholder="Search by campaign name or target URL..."
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <select className="enterprise-input w-full md:w-40 appearance-none" style={glassInput} value={filter} onChange={(e) => { setFilter(e.target.value); setPage(1); }}>
      ...
      {/* Main Table */}
      <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full">
          ...
          <table className="enterprise-table min-w-[1100px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="6" className="text-center py-16">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm font-medium text-white/40">No tracking links found</p>
                ...
                paginatedLinks.map((link, rowIdx) => {
                  ...
                  const statusBg = status === 'active' ? 'rgba(16,185,129,0.1)' : status === 'paused' ? 'rgba(245,158,11,0.1)' : 'rgba(239,68,68,0.1)';
                    ...
                    >
                      <td className="align-top py-4">
                        <div className="font-semibold text-white text-sm flex items-center">
                          ...
                          {link.isDefault && <span className="ml-2 text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider bg-[#10b981]/10 text-[#10b981]">Primary</span>}
                        </div>
                        <div className="text-xs text-white/40 mt-1 truncate max-w-[200px]" title={targetUrl}>
                          {targetUrl || 'N/A'}
                        </div>
                        <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                          <span className="text-[10px] font-mono px-1.5 py-0.5 rounded flex items-center text-white/40" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
                            <Code className="w-3 h-3 mr-1" />{sendCode}
                          </span>
                          {hasPixels && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider bg-[#3b82f6]/10 text-[#3b82f6]">Pixels</span>}
                          {hasOG && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider bg-[#10b981]/10 text-[#10b981]">OG</span>}
                          {hasRules && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider bg-[#f59e0b]/10 text-[#f59e0b]">Rules</span>}
                        </div>
                        <InboxScoreBadge linkId={link.id} />
                      </td>

                      <td className="align-top py-4 space-y-2">
                        <div className="flex items-center justify-between px-2 py-1.5 rounded text-xs" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <div className="flex items-center overflow-hidden mr-2">
                            <LinkIcon className="w-3.5 h-3.5 text-[#3b82f6] mr-1.5 shrink-0" />
                            <span className="font-mono truncate text-white/40" title={trackingUrl}>{trackingUrl || 'Pending...'}</span>
                          </div>
                          <button onClick={() => handleAction('Copy Link', link)} className="shrink-0 p-1 rounded hover:bg-white/5 text-white/40 hover:text-white" title="Copy tracking link">
                            <Copy className="w-3.5 h-3.5" />
                          </button>
                        </div>
                        <div className="flex items-center justify-between px-2 py-1.5 rounded text-xs" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
                          <div className="flex items-center overflow-hidden mr-2">
                            ...
                            <span className="font-mono truncate text-white/40" title={pixelUrl}>{pixelUrl || 'Pixel not configured'}</span>
                          </div>
                          <button onClick={() => handleAction('Copy Pixel', link)} className="shrink-0 p-1 rounded hover:bg-white/5 text-white/40 hover:text-white" title="Copy pixel URL">
                            <Copy className="w-3.5 h-3.5" />
                          </button>
                        </div>
                      </td>
                      ...
                      <td className="align-top py-4 text-center">
                        <div className="flex justify-center items-end gap-3">
                          <div className="flex flex-col items-center">
                            ...
                            <span className="tabular-nums-custom font-bold text-white">{visitors}</span>
                            <span className="text-[10px] uppercase tracking-widest text-white/40 mt-0.5">Real</span>
                          </div>
                          ...
                          <div className="flex flex-col items-center">
                            ...
                            <span className="tabular-nums-custom font-bold text-white">{emails}</span>
                            <span className="text-[10px] uppercase tracking-widest text-white/40 mt-0.5">Emails</span>
                          </div>
                          <div className="w-px h-8" style={{ background: 'rgba(255,255,255,0.06)' }} />
                          <div className="flex flex-col items-center">
                            <Shield className="w-4 h-4 text-[#ef4444] mb-1 opacity-80" />
                            <span className="tabular-nums-custom font-bold text-white">{bots}</span>
                            <span className="text-[10px] uppercase tracking-widest text-white/40 mt-0.5">Bots</span>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="align-top py-4">
                        <div className="flex justify-end items-center gap-0.5 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleAction('QR', link)} className="p-1.5 rounded-md hover:bg-[rgba(59,130,246,0.1)] text-white/40 hover:text-[#3b82f6]" title="QR Code"><QrCode className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Health', link)} className="p-1.5 rounded-md hover:bg-[rgba(16,185,129,0.1)] text-white/40 hover:text-[#10b981]" title="Health Monitor"><Heart className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Pixels', link)} className="p-1.5 rounded-md hover:bg-[rgba(245,158,11,0.1)] text-white/40 hover:text-[#f59e0b]" title="Pixel Injection"><Zap className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('OG', link)} className="p-1.5 rounded-md hover:bg-[rgba(139,92,246,0.1)] text-white/40 hover:text-[#8b5cf6]" title="Open Graph"><Share2 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Routing', link)} className="p-1.5 rounded-md hover:bg-[rgba(16,185,129,0.1)] text-white/40 hover:text-[#10b981]" title="Routing Rules"><Filter className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Edit', link)} className="p-1.5 rounded-md hover:bg-[rgba(59,130,246,0.1)] text-white/40 hover:text-[#3b82f6]" title="Edit"><Edit className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Regenerate', link)} className="p-1.5 rounded-md hover:bg-[rgba(245,158,11,0.1)] text-white/40 hover:text-[#f59e0b]" title="Regenerate"><RefreshCw className="w-3.5 h-3.5" /></button>
                          <button onClick={() => handleAction('Delete', link)} className="p-1.5 rounded-md hover:bg-[rgba(239,68,68,0.1)] text-white/40 hover:text-[#ef4444]" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                    </motion.tr>
                  );
      ...
      {showFormModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setShowFormModal(false)}>
          <div style={{ ...glassCard, overflow: 'hidden' }} className="w-full max-w-xl mx-4 p-6 relative shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowFormModal(false)} className="p-1.5 rounded-md hover:bg-white/5 text-white/40">
                <X className="w-5 h-5" />
              </button>
            </div>
            <CreateLinkForm
