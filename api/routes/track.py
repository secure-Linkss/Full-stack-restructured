...
def create_tracking_notification(link, event):
    ...
    try:
        ...
        # Determine notification message based on event status
        if event.status == "blocked" or event.blocked_reason:
            title = "🚫 Access Blocked"
            message = f"A visitor from {event.country or 'Unknown'} was blocked from accessing your link '{link.campaign_name}'."
            notif_type = "security"
        elif event.on_page:
            title = "✅ Visitor On Page"
            message = f"A visitor from {event.city or 'Unknown'}, {event.country or 'Unknown'} is actively browsing your target page for link '{link.campaign_name}'."
            notif_type = "success"
        elif event.redirected:
            title = "🔗 Link Clicked"
            message = f"Your link '{link.campaign_name}' received a click from {event.city or 'Unknown'}, {event.country or 'Unknown'}."
            notif_type = "info"
        elif event.email_opened:
            title = "📧 Email Opened"
            message = f"Your tracking pixel in '{link.campaign_name}' was opened by {event.captured_email or 'a recipient'}."
            notif_type = "success"
        else:
            title = "👁️ New Click"
            message = f"Your link '{link.campaign_name}' received a click from {event.country or 'Unknown'}."
            notif_type = "info"
...
def track_click(short_code):
    ...
    try:
        ...
        if link.expires_at and link.expires_at < now:
            # Auto-deactivate and redirect/return 410
            link.status = 'expired'
            db.session.commit()
        ...
        try:
            ...
            create_tracking_notification(link, event)
